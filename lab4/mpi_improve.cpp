...
    // 所有进程参与计算
    int64 cur_mod = 0;
    if (rank == 0)       cur_mod = mod1;
    else if (rank == 1)  cur_mod = mod2;
    else if (rank == 2)  cur_mod = mod3;
    else return;  // 多余进程退出
...
    // 结果收集
    if (rank == 0) {
        vector<int64> r1 = std::move(a_pad);  // 0号进程自己的结果
        vector<int64> r2(len), r3(len);
        MPI_Recv(r2.data(), len, MPI_LONG_LONG, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(r3.data(), len, MPI_LONG_LONG, 2, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
