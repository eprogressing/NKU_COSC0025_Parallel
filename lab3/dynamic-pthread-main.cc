...
// inv(mod1*mod2 % mod3, mod3)
static const int64 inv_m12_mod_m3 = [](){
    int64 m12 = (int64)(mod1) * mod2 % mod3;
...
struct NTTArgs { int* a, *b, *result; int n, mod; };
void* ntt_thread(void* _args) {
    auto args = (NTTArgs*)_args;
    int n = args->n, mod = args->mod;
    vector<int> A(args->a, args->a + n), B(args->b, args->b + n);
    ntt(A.data(), n, mod, 1);
    ntt(B.data(), n, mod, 1);
    for (int i = 0; i < n; ++i)
        A[i] = (__int128)A[i] * B[i] % mod;
    ntt(A.data(), n, mod, -1);
    memcpy(args->result, A.data(), n * sizeof(int));
    return nullptr;
}
...
void poly_multiply(int *a, int *b, int *out, int n, int P) {
    int len = 1;
    while (len < 2 * n) len <<= 1;
    vector<int> a_pad(len), b_pad(len);
    copy(a, a + n, a_pad.begin());
    copy(b, b + n, b_pad.begin());
    vector<int> r1(len), r2(len), r3(len);
    NTTArgs args[3] = {{a_pad.data(), b_pad.data(), r1.data(), len, (int)mod1},
                       {a_pad.data(), b_pad.data(), r2.data(), len, (int)mod2},
                       {a_pad.data(), b_pad.data(), r3.data(), len, (int)mod3}};
    pthread_t th[3];
    for (int i = 0; i < 3; ++i) pthread_create(&th[i], nullptr, ntt_thread, &args[i]);
    for (int i = 0; i < 3; ++i) pthread_join(th[i], nullptr);
    crt_merge(r1.data(), r2.data(), r3.data(), out, n, P);
}
